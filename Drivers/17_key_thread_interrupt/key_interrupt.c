...
{
	struct gpio_key *gpios_key1 = data;

	printk("gpio_key_thread_func  gpio_key_rk3399: val %d\r\n", gpio_get_value(gpios_key1->gpio));
...
static int rk3399_key_probe(struct platform_device *pdev) {
	...
	for(i=0; i<count; i++) {
		...
		err = request_threaded_irq(gpios_key[i].irq, gpio_key_rk3399, gpio_key_thread_func, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "rk3399_key", &gpios_key[i]);
	}
